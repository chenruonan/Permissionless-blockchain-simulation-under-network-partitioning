...
for ip in "${ip_list[@]}"; do
    ...
    # 限制上传带宽
    ssh root@$ip "export CNT=$cnt;export IPSRC=$ip; export IPDST=$CIDR2;export SPEED=$SPEED; bash -s" < /usr/local/myproj/bitcoinLab/setBandwidth.sh &
    echo "$ip is in $CIDR1"
    elif ip_in_subnet "$ip" "$CIDR2";then
    # 限制上传带宽
    ssh root@$ip "export CNT=$cnt;export IPSRC=$ip; export IPDST=$CIDR1;export SPEED=$SPEED; bash -s" < /usr/local/myproj/bitcoinLab/setBandwidth.sh &
    echo "$ip is in $CIDR2"
    fi
done
