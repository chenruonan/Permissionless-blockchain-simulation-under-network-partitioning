...
execute_command_add_PID geth_$node_num.log ./geth.sh $DATADIR/geth_datadir$node_num $((EL_base_network + $node_num)) $((EL_base_http + $node_num)) $((EL_base_auth_http + $node_num)) $genesis_file
sleeping 20
...
execute_command_add_PID beacon_node_$node_num.log ./beacon_node.sh $SAS -d $DEBUG_LEVEL $DATADIR/node_$node_num $((BN_udp_tcp_base + $node_num)) $((BN_http_port_base + $node_num)) http://localhost:$((EL_base_auth_http + $node_num)) $secret
