...
#     echo "$!" >> $PID_FILE
# }
echo "get genesisTime"
GENESIS_TIME=$(lcli pretty-ssz --testnet-dir . BeaconState ./genesis.ssz | jq | grep -Po 'genesis_time": "\K.*\d')
echo $GENESIS_TIME
...
execute_command geth_$node_num.log ./geth.sh $DATADIR/geth_datadir$node_num $((EL_base_network + $node_num)) $((EL_base_http + $node_num)) $((EL_base_auth_http + $node_num)) $genesis_file $el_bootnode_address $node_num
sleeping 10
...
execute_command beacon_node_$node_num.log ./beacon_node.sh $SAS -d $DEBUG_LEVEL $DATADIR/nodes/node_$node_num $((BN_udp_tcp_base + $node_num)) $((BN_http_port_base + $node_num)) http://localhost:$((EL_base_auth_http + $node_num)) $secret $ipaddress $bootnode_enr

#start requested number of validator clients
execute_command validator_node_$node_num.log ./validator_client.sh $BUILDER_PROPOSALS -d $DEBUG_LEVEL $DATADIR/nodes/node_$node_num http://localhost:$((BN_http_port_base + $node_num))
