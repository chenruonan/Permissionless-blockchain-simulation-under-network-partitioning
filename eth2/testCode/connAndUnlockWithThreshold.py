...
def deployContract(node: Web3):
    with open("erc20_dsWithThreshold.solidi") as f:
...
def simuT(total_contract_list):
    while(1):
        for contract in total_contract_list:
...
if __name__=='__main__':
    ...
    workbook = openpyxl.load_workbook("/usr/local/myproj/ethereumLab/testCode/dsWithThresholdTest.xlsx")
    sheet_name="256kb"
            # 检查工作表是否已经存在
    ...
    subNetTrans=[]
    for i in range(1):
        mainNetTrans.append(total_contract_list[0].functions.spend(1,63).transact().hex())# 主网操作
        subNetTrans.append(total_contract_list[57].functions.spend(2,63).transact().hex())# 子网操作
    ...
    while(1):
        ...
        if mainNetCostByMain+subNetCostBySub>63:
            ...
            workbook.save("/usr/local/myproj/ethereumLab/testCode/dsWithThresholdTest.xlsx")
            workbook.close()
            break
        else:
