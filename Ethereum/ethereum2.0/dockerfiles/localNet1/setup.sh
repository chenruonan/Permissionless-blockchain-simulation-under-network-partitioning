...
# get our genesis file(input through command line)
genesis_file=${@:$OPTIND+0:1}

# Init some constants
PID_FILE=$TESTNET_DIR/PIDS.pid # contains the process ID of the node we will start
LOG_DIR=$TESTNET_DIR        # set the directory for putting the log files

mkdir -p $LOG_DIR # create the directory for storing log files
